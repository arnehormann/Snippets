...
### query
SELECT
  si.id, si.sites, si.refs,
  @refdate as qdate,
  ifnull(ac.entries / @window, @nodata) as event_density,
  if((ac.announcements / ac.entries) < @min_density, @defect, @ok) as announcements
FROM (
    SELECT
  ...
  LEFT JOIN (
    SELECT ce.category_id, count(ce.entry) entries, count(an.content) announcements
    FROM (
      SELECT ifnull(@category:=category_id, 0) uid, min(creation) mindate, count(*) = @window valid
      FROM (
        SELECT category_id, creation
        FROM category_entries
        WHERE creation BETWEEN @mindate AND @maxdate
        ORDER BY category_id ASC, creation DESC
      ) tmp
      WHERE (category_id != @category AND 0 = (
        SELECT if(@idx > 0, @idx:=0, @idx))) OR (@window > (SELECT @idx:=@idx+1) OR false
      )
      GROUP BY uid
    ) mindate
